...
- ⚙️ **豐富的步驟與逾時轉向處置 (Advanced Action & Timeout Control)**
  - 支援「僅點擊中心點」、「僅發送按鍵 (含自訂按住時間/延遲/重複次數)」、「貼上字串內容」或「點擊 + 發送按鍵」等多種觸發動作。
  - 🎨 **多色漸層/色調容許檢測 (Multi-Color Matching)**：支援逗號分隔多組 HEX 色碼（如 `0xFFD700,0xFFDF00`），完美契合動態遊戲 UI 漸層與微調需求。
  - ⌨️ **獨立按鍵發送動作 (Key Press Action)**：內建常用按鍵快選、自訂按鍵名稱、按住時間 (ms)、釋放後延遲 (ms) 與連續重複次數 (1~99 次)。
  - 📝 **獨立字串貼上動作 (Text Paste Action)**：支援直接貼上分享代碼或文字內容並自訂完成延遲。
  - 🔁 **支援步驟內循環轉向控制 (Loop Goto Step)**：例如在步驟 10 設定跳轉回步驟 8，重複執行 N 次後自動解鎖推進至步驟 11。
  - ⏱ **支援純等待秒數 (Wait Action)** 與逾時轉向機制 (0-300 秒)，逾時可自動發送特定按鍵，並選擇重試本步驟、推進下一步或跳轉至指定步驟。

- 📥 **完整的 INI 設定檔匯入與匯出 (INI Configuration Management)**
  - 隨時匯出備份巨集設定，或從 INI 設定檔快速匯入巨集資料庫。
  - 內建與 `fh6.ahk` 對應的 Forza Horizon 6 (FH6) 全套 6 大行程預設範本（賺技能點、點技能、快速買車、勁敵刷錢、連點 Enter、長按油門）。
...
### 主懸浮列操作
- **⚙ (齒輪圖示)**：開啟/關閉多群組巨集編輯器。
- **圖示按鈕 (如 ⚔️ 賺技能點, ⚡️ 點技能, 🚗 買車, 🎖️ 勁敵 等)**：點擊即可啟動或停止該群組巨集執行。
- **⏏ (離開圖示)**：停止巨集並關閉程式。
- **右鍵選單**：在懸浮列任意位置點擊右鍵，可呼出功能選單（編輯器、匯入/匯出 INI、離開）。

### 巨集編輯器操作
1. **切換群組**：點擊頂部 `📋 選擇巨集` 下拉選單 (DropDownList) 即可即時切換當前編輯的群組。
2. **新增與編輯群組**：點擊 `➕ 新增`，可經由 DropDownList 下拉選單選擇複製來源群組、選擇 Emoji 圖示及是否顯示於主懸浮列。
3. **新增動作步驟**：
   - `🎯 圈選顏色`：在螢幕上按住滑鼠左鍵拖曳圈選目標區域，支援多色值、容許度、動作處置與逾時轉向。
   - `⌨️ +按鍵`：新增按鍵發送，可設定按住時間 (ms)、延遲 (ms) 與重複次數。
   - `📝 +貼字串`：新增剪貼簿文字貼上動作。
   - `⏱ +等待`：新增純秒數延遲等待。
   - `🔁 +步驟循環`：設定返回指定步驟與重複次數。
...
├── macro_config.ini     # 預設載入之巨集設定檔 (含 FH6 六大完整行程)
├── fh6_macro.ini        # FH6 專用行程備份設定檔
├── README.md            # 專案詳細說明文件
